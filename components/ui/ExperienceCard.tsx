...
const ExperienceCard = ({ exp }: { exp: (typeof experiences)[number] }) => {
  ...
  return (
    <motion.a
      ...
      className={`group flex items-start gap-4 p-4 text-sm/7 focus:outline-none max-lg:flex-col sm:p-6 lg:grid lg:grid-cols-[auto_1fr] ${hoverColorSVG}`}
      role="tab"
      type="button"
      aria-selected="false"
    ...
    >
      ...
      <div className="text-center xl:text-left">
        <div className="flex justify-between gap-4 max-lg:w-full max-lg:flex-col max-lg:items-center">
          <div>
            <p className="font-mono text-sm font-semibold tracking-widest uppercase">
              {exp.label}
            </p>
            <p className="text-current/60">{exp.profile}</p>
          </div>
          <div>
            <p className="text-sm text-current/60 group-hover:text-current">
